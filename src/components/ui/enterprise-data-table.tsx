...
import {
import { DaisySelect, DaisySelectTrigger, DaisySelectContent, DaisySelectItem, DaisySelectValue, DaisyDropdownMenu, DaisyDropdownMenuTrigger, DaisyDropdownMenuContent, DaisyDropdownMenuItem, DaisyCalendar } from '@/components/ui/daisy-components';
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuLabel,
...
const StatusBadgeCell: React.FC<{ value: any; colorMapping?: Record<string, string> }> = ({ 
  ...
  return (
    <DaisyBadge 
      ...
      className={cn(
        "text-xs font-medium",
        getVariant(value) === 'destructive' && "bg-semantic-error text-white",
        getVariant(value) === 'outline' && "bg-semantic-warning text-white",
        getVariant(value) === 'secondary' && "bg-semantic-success text-white"
      )} >
  {value}
</DaisyBadge>
    </DaisyBadge>
      ...
      {filters.length > 0 && (
        <div className="flex items-center space-x-enterprise-2 p-enterprise-3 bg-surface-secondary border-b border-border">
          ...
          {filters.map((filter, index) => {
            ...
            return (
              <DaisyBadge key={index} variant="outline" className="text-caption border-border" >
        ...
        <div className="flex items-center justify-between p-enterprise-4 border-t border-border bg-surface-secondary rounded-b-lg">
          <div className="flex items-center space-x-enterprise-4">
            ...
            <DaisySelect value={String(pageSize)} onValueChange={(value) => onPageSizeChange?.(Number(value))} />
              ...
              <DaisySelectContent >
                  <DaisySelectItem value="25">25</DaisySelectItem>
