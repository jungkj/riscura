...
const StatusBadgeCell: React.FC<{ value: any; colorMapping?: Record<string, string> }> = ({
  ...
  return (
    <DaisyBadge
      ...
      className={cn('text-xs font-medium', customColor && `bg-${customColor} text-white`)}
    >
      {value}
    </DaisyBadge>
  );
...
export const EnterpriseDataTable = <T extends Record<string, any>>({
  ...
  const handleFilter = useCallback(
    (columnKey: string, operator: FilterState['operator'], value: any) => {
      setFilters((prev) => {
  ...
  const toggleColumnVisibility = useCallback((columnKey: string) => {
    setColumns((prev) =>
      prev.map((col) => (col.key === columnKey ? { ...col, hidden: !col.hidden } : col))
    );
  }, []);
              ...
              onChange={(e) = />
{
          ...
          {selectedRows.size > 0 && bulkActions.length > 0 && (
            <div className="flex items-center space-x-enterprise-2">
              <span className="text-body-sm text-text-secondary">{selectedRows.size} selected</span>
              {bulkActions.map((action) => (
                <DaisyButton
                  key={action.key}
                  variant={action.variant === 'destructive' ? 'destructive' : ('outline' as any)}
                  size="sm"
                  onClick={() => {
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              {savedViews.map((view) => (
                <DropdownMenuItem key={view.id} onClick={() => setActiveView(view)}>
                  {view.name}
                  {view.isDefault && (
                    <DaisyBadge variant="secondary" className="ml-auto text-xs">
                      Default
                    </DaisyBadge>
                  )}
                </DropdownMenuItem>
                ...
                <tr
                  ...
                  className={cn(
                    ...
                    selectedRows.has(rowIndex) &&
                      'bg-interactive-primary/5 border-interactive-primary/20'
                  )}
