...
test.describe('Risk Management API Integration', () => {
  ...
  test('CRUD operations for risks', async () => {
    ...
    const updateResponse = await helper.makeAuthenticatedRequest('PUT', `/risks/${riskId}`, updateData);
  ...
  test('CRUD operations for controls', async () => {
    ...
    const updateResponse = await helper.makeAuthenticatedRequest('PUT', `/controls/${controlId}`, updateData);
    expect(updateResponse.status()).toBe(200);
    ...
    // Delete control
    const deleteResponse = await helper.makeAuthenticatedRequest('DELETE', `/controls/${controlId}`);
    expect(deleteResponse.status()).toBe(200);

    // Verify deletion
    const verifyDeleteResponse = await helper.makeAuthenticatedRequest('GET', `/controls/${controlId}`);
    expect(verifyDeleteResponse.status()).toBe(404);
  });
  ...
  test('Risk search and filtering', async () => {
    ...
    // Test search
    const searchResponse = await helper.makeAuthenticatedRequest('GET', '/risks?search=cybersecurity');
    ...
    const filterResponse = await helper.makeAuthenticatedRequest('GET', '/risks?category=Technology');
...
test.describe('Control Management API Integration', () => {
  ...
  test('Control lifecycle management', async () => {
    ...
    const implementResponse = await helper.makeAuthenticatedRequest('POST', `/controls/${controlId}/implement`, {
    ...
    const testResponse = await helper.makeAuthenticatedRequest('POST', `/controls/${controlId}/test`, {
    ...
    const effectivenessResponse = await helper.makeAuthenticatedRequest('GET', `/controls/${controlId}/effectiveness`);
  ...
  test('Control-Risk mapping', async () => {
    ...
    // Map control to risk
    const mappingResponse = await helper.makeAuthenticatedRequest('POST', `/risks/${risk.id}/controls`, {
    ...
    // Verify mapping
    const riskWithControlsResponse = await helper.makeAuthenticatedRequest('GET', `/risks/${risk.id}?include=controls`);
...
test.describe('Document Management API Integration', () => {
  ...
  test('Document upload and download workflow', async () => {
    ...
    const metadataResponse = await helper.makeAuthenticatedRequest('GET', `/documents/${documentId}`);
    ...
    // Download document
    const downloadResponse = await helper.makeAuthenticatedRequest('GET', `/documents/${documentId}/download`);
    ...
    // Delete document
    const deleteResponse = await helper.makeAuthenticatedRequest('DELETE', `/documents/${documentId}`);
    expect(deleteResponse.status()).toBe(200);
  });
  ...
  test('Document version management', async () => {
    ...
    // Upload new version
    const versionResponse = await helper.page.request.post(`${API_BASE_URL}/documents/${documentId}/versions`, {
      headers: {
        'Authorization': `Bearer ${helper.authToken}`
      },
      multipart: {
    ...
    // Get version history
    const versionsResponse = await helper.makeAuthenticatedRequest('GET', `/documents/${documentId}/versions`);
    ...
    const version1Response = await helper.makeAuthenticatedRequest('GET', `/documents/${documentId}/versions/1/download`);
...
test.describe('Reporting API Integration', () => {
  ...
  test('Report generation workflow', async () => {
    ...
    const generateResponse = await helper.makeAuthenticatedRequest('POST', '/reports/generate', reportRequest);
    ...
    while (reportStatus === 'GENERATING' && attempts < 10) {
      ...
      const statusResponse = await helper.makeAuthenticatedRequest('GET', `/reports/${reportId}/status`);
    ...
    const completedReportResponse = await helper.makeAuthenticatedRequest('GET', `/reports/${reportId}`);
    ...
    const exportResponse = await helper.makeAuthenticatedRequest('GET', `/reports/${reportId}/export/pdf`);
  ...
  test('Report templates and customization', async () => {
    ...
    const createTemplateResponse = await helper.makeAuthenticatedRequest('POST', '/reports/templates', customTemplate);
    ...
    const templateReportResponse = await helper.makeAuthenticatedRequest('POST', '/reports/generate', reportWithTemplate);
...
test.describe('AI Integration API', () => {
  ...
  test('AI risk analysis', async () => {
    ...
    const analysisResponse = await helper.makeAuthenticatedRequest('POST', '/ai/analyze', analysisRequest);
  ...
  test('AI document analysis', async () => {
    const testContent = 'This is a security policy document that outlines access controls and data protection measures';
    ...
    const docAnalysisResponse = await helper.makeAuthenticatedRequest('POST', '/ai/document/analyze', docAnalysisRequest);
  ...
  test('AI conversation history', async () => {
    ...
    const historyResponse = await helper.makeAuthenticatedRequest('GET', `/ai/conversations/${sessionId}`);
...
test.describe('API Performance and Load Testing', () => {
  ...
  test('Pagination performance with large datasets', async () => {
    ...
    for (const pageSize of pageSizes) {
      const startTime = Date.now();
      const response = await helper.makeAuthenticatedRequest('GET', `/risks?page=1&limit=${pageSize}`);
      const endTime = Date.now();
  ...
  test('Search performance with complex queries', async () => {
    ...
    for (const query of searchQueries) {
      const startTime = Date.now();
      const response = await helper.makeAuthenticatedRequest('GET', `/risks?search=${encodeURIComponent(query)}`);
      const endTime = Date.now();
  ...
  test('Bulk operations performance', async () => {
    ...
    const startTime = Date.now();
    const bulkResponse = await helper.makeAuthenticatedRequest('POST', '/risks/bulk', { risks: bulkRisks });
    const endTime = Date.now();
