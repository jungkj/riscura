...
class MultiTenantRiscuraApp {
  constructor(public page: Page, public tenantId: string) {}
  ...
  async setupTenant(tenantData: any) {
    ...
    await this.page.selectOption('[data-testid="industry-select"]', tenantData.organization.industry);
  ...
  async createSensitiveRisk(riskData: any) {
    ...
    await this.page.fill('[data-testid="risk-description"]', 'CONFIDENTIAL: This contains sensitive tenant data');
  ...
  async searchForCrossTenantData(searchTerm: string) {
    ...
    const resultTexts = await this.page.locator('[data-testid="search-result-item"]').allTextContents();
...
test.describe('Multi-Tenant Security and Data Isolation', () => {
  ...
  test('Complete tenant setup and isolation validation', async () => {
    ...
    await test.step('Verify search does not return cross-tenant results', async () => {
      ...
      const searchResults = await tenantAApp.searchForCrossTenantData(tenantBData.sensitiveData.riskTitle);
  ...
  test('User permission boundaries within tenant', async () => {
    ...
    const canDeleteOrg = await tenantAApp.page.locator('[data-testid="delete-organization"]').isVisible();
  ...
  test('Session isolation between tenants', async () => {
    ...
    const tenantAOrgName = await tenantAApp.page.locator('[data-testid="organization-name"]').textContent();
    const tenantBOrgName = await tenantBApp.page.locator('[data-testid="organization-name"]').textContent();
  ...
  test('Audit log isolation', async () => {
    ...
    const auditEntries = await tenantAApp.page.locator('[data-testid="audit-entry"]').allTextContents();
