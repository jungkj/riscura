...
test.describe('Critical User Journeys', () => {
  ...
  test.describe('New User Registration Journey', () => {
    test('complete new user onboarding flow', async () => {
      ...
      await page.fill('[data-testid="first-name-input"], input[name="firstName"], input[placeholder*="first name" i]', 'Test');
      await page.fill('[data-testid="last-name-input"], input[name="lastName"], input[placeholder*="last name" i]', 'User');
      await page.fill('[data-testid="email-input"], input[type="email"], input[name="email"]', uniqueEmail);
      await page.fill('[data-testid="password-input"], input[type="password"], input[name="password"]', 'TestPassword123!');
      await page.fill('[data-testid="organization-input"], input[name="organization"], input[placeholder*="organization" i]', 'Test Organization');
      ...
      await page.click('[data-testid="register-button"], button[type="submit"], button:has-text("Register"), button:has-text("Sign Up")');
      ...
      if (currentUrl.includes('/dashboard')) {
        ...
        await expect(page.locator('[data-testid="user-menu"], .user-menu, [aria-label*="user"]')).toBeVisible({ timeout: 10000 });
    ...
    test('registration form validation', async () => {
      ...
      await expect(page.locator('[data-testid*="error"], .error, .text-red')).toBeVisible({ timeout: 5000 });
      ...
      await expect(page.locator('text=invalid email, text=valid email')).toBeVisible({ timeout: 5000 });
    });
  });
  ...
  test.describe('Dashboard Navigation Journey', () => {
    ...
    test('navigate through main dashboard sections', async () => {
      ...
      for (const section of navSections) {
        ...
        const navLink = page.locator(`a:has-text("${section.text}"), [data-testid="${section.text.toLowerCase()}-nav"], nav a[href*="${section.url}"]`).first();
  ...
  test.describe('Risk Management Journey', () => {
    ...
    test('create new risk assessment', async () => {
      ...
      const createButton = page.locator('[data-testid="create-risk"], button:has-text("Create"), button:has-text("New Risk"), .btn-primary').first();
      ...
      await page.fill('[data-testid="risk-title"], input[name="title"], input[placeholder*="title" i]', riskTitle);
      await page.fill('[data-testid="risk-description"], textarea[name="description"], textarea[placeholder*="description" i]', 'This is a test risk created by automated testing');

      // Select risk category if dropdown exists
      const categorySelect = page.locator('[data-testid="risk-category"], select[name="category"], select:has(option)').first();
      if (await categorySelect.isVisible()) {
        await categorySelect.selectOption({ index: 1 });
      }

      // Set risk likelihood and impact if controls exist
      const likelihoodControl = page.locator('[data-testid="likelihood"], input[name="likelihood"], select[name="likelihood"]').first();
      const impactControl = page.locator('[data-testid="impact"], input[name="impact"], select[name="impact"]').first();
      ...
      // Submit the form
      await page.click('[data-testid="save-risk"], button[type="submit"], button:has-text("Save"), button:has-text("Create")');
    ...
    test('risk list filtering and search', async () => {
      ...
      const searchInput = page.locator('[data-testid="search"], input[type="search"], input[placeholder*="search" i]').first();
      ...
      const filterButton = page.locator('[data-testid="filter"], button:has-text("Filter"), .filter-toggle').first();
  ...
  test.describe('Data Persistence Journey', () => {
    test('data persists across sessions', async () => {
      ...
      await page.fill('textarea[name="description"], [data-testid="risk-description"]', 'Persistence test data');
    ...
    test('form data recovery after page refresh', async () => {
      ...
      await page.fill('textarea[name="description"], [data-testid="risk-description"]', 'This is draft content');
  ...
  test.describe('Mobile Responsiveness Journey', () => {
    ...
    test('mobile navigation functionality', async () => {
      ...
      if (await mobileMenuToggle.isVisible()) {
        ...
        // Verify menu opened
        const mobileMenu = page.locator('[data-testid="mobile-nav"], .mobile-navigation, .mobile-menu').first();
  ...
  test.describe('Error Handling Journey', () => {
    ...
    test('form validation error display', async () => {
      ...
      // Check for validation errors
      await expect(page.locator('[data-testid*="error"], .error, .text-red, .invalid-feedback')).toBeVisible({ timeout: 5000 });
