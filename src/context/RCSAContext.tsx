...
interface RCSAContextActions {
  ...
  updateControlEffectiveness: (riskId: string, controlId: string, effectiveness: number) => Promise<void>;
  ...
  switch (action.type) {
    ...
    case 'UPDATE_RISK':
      return {
        ...
        currentRisk: state.currentRisk?.id === action.payload.id ? action.payload : state.currentRisk
      };
    ...
    case 'UPDATE_CONTROL':
      return {
        ...
        currentControl: state.currentControl?.id === action.payload.id ? action.payload : state.currentControl
      };
    ...
    case 'REMOVE_CONTROL_RISK_MAPPING':
      return {
        ...state,
        controlRiskMappings: state.controlRiskMappings.filter(mapping =>
          !(mapping.riskId === action.payload.riskId && mapping.controlId === action.payload.controlId)
        )
      };
...
export function RCSAProvider({ children }: { children: React.ReactNode }) {
  ...
  const navigateToRisk = useCallback(async (riskId: string, fromContext?: NavigationContext) => {
    setLoading(true);
    try {
  ...
  const navigateToControl = useCallback(async (controlId: string, fromContext?: NavigationContext) => {
    setLoading(true);
    try {
  ...
  const navigateToAssessment = useCallback(async (assessmentId: string, fromContext?: NavigationContext) => {
  ...
  const createRisk = useCallback(async (riskData: CreateRiskRequest): Promise<Risk> => {
    setLoading(true);
    try {
  ...
  const updateRisk = useCallback(async (id: string, updates: UpdateRiskRequest): Promise<Risk> => {
    setLoading(true);
    try {
  ...
  const createControl = useCallback(async (controlData: CreateControlRequest): Promise<Control> => {
    setLoading(true);
    try {
  ...
  const updateControl = useCallback(async (id: string, updates: UpdateControlRequest): Promise<Control> => {
    setLoading(true);
    try {
  ...
  const deleteControl = useCallback(async (id: string): Promise<void> => {
    setLoading(true);
    try {
  ...
  const mapControlToRisk = useCallback(async (riskId: string, controlId: string, effectiveness = 0.5) => {
    setLoading(true);
    try {
  ...
  const unmapControlFromRisk = useCallback(async (riskId: string, controlId: string) => {
    setLoading(true);
    try {
  ...
  const updateControlEffectiveness = useCallback(async (riskId: string, controlId: string, effectiveness: number) => {
    setLoading(true);
    try {
      const response = await rcsaApiClient.updateControlEffectiveness(riskId, controlId, effectiveness);
      if (response.success && response.data) {
  ...
  const bulkMapControls = useCallback(async (riskId: string, controlIds: string[]) => {
    setLoading(true);
    try {
  ...
  const bulkUpdateEffectiveness = useCallback(async (updates: EffectivenessUpdate[]) => {
    setLoading(true);
    try {
  ...
  const getRelatedControls = useCallback((riskId: string): Control[] => {
    const mappings = state.controlRiskMappings.filter(m => m.riskId === riskId);
    return state.controls.filter(control => 
      mappings.some(m => m.controlId === control.id)
    );
  }, [state.controlRiskMappings, state.controls]);

  const getRelatedRisks = useCallback((controlId: string): Risk[] => {
  ...
  return (
    <RCSAContext.Provider value={contextValue}>
      {children}
    </RCSAContext.Provider>
  );
}
