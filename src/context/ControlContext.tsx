...
interface ControlContextType extends ControlState {
  ...
  mapControlToRisk: (controlId: string, riskId: string, mapping: Omit<ControlRiskMapping, 'controlId' | 'riskId'>) => Promise<void>;
  ...
  updateControlEffectiveness: (controlId: string, riskId: string, effectiveness: number) => Promise<void>;
  ...
  completeControlTest: (controlId: string, results: { effectiveness: number; evidence?: Document[] }) => Promise<void>;
  ...
  switch (action.type) {
    ...
    case 'UPDATE_CONTROL':
      return {
        ...
        selectedControl: state.selectedControl?.id === action.payload.id ? action.payload : state.selectedControl,
        loading: false,
      };
    ...
    case 'DELETE_CONTROL':
      return {
        ...
        selectedControl: state.selectedControl?.id === action.payload ? null : state.selectedControl,
    ...
    case 'DELETE_CONTROLS':
      return {
        ...
        selectedControl: action.payload.includes(state.selectedControl?.id || '') ? null : state.selectedControl,
        selectedControls: [],
        loading: false,
      };
    ...
    case 'REMOVE_CONTROL_RISK_MAPPING':
      return {
        ...
        controlRiskMappings: state.controlRiskMappings.filter(
          mapping => !(mapping.controlId === action.payload.controlId && mapping.riskId === action.payload.riskId)
        ),
      };
...
export const ControlProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const mapControlToRisk = async (controlId: string, riskId: string, mapping: Omit<ControlRiskMapping, 'controlId' | 'riskId'>) => {
  ...
  const updateControlEffectiveness = async (controlId: string, riskId: string, effectiveness: number) => {
    const mapping = state.controlRiskMappings.find(m => m.controlId === controlId && m.riskId === riskId);
    if (mapping) {
      const updated = { ...mapping, effectivenessRating: effectiveness, lastTested: new Date().toISOString() };
  ...
  const completeControlTest = async (controlId: string, results: { effectiveness: number; evidence?: Document[] }) => {
    const control = getControl(controlId);
    if (control) {
      const updated = {
        ...control,
        effectiveness: results.effectiveness > 0.8 ? 'high' as const : 
                      results.effectiveness > 0.6 ? 'medium' as const : 'low' as const,
  ...
  const getFilteredControls = (): Control[] => {
    ...
    if (state.filters.effectiveness) {
      filtered = filtered.filter(control => control.effectiveness === state.filters.effectiveness);
    }
  ...
  const getControlStats = () => {
    ...
    const byType = controls.reduce((acc, control) => {
      acc[control.type] = (acc[control.type] || 0) + 1;
      return acc;
    }, {} as Record<Control['type'], number>);
    
    const byEffectiveness = controls.reduce((acc, control) => {
    ...
    return { total, byType, byEffectiveness, byStatus, averageEffectiveness, overdueTests, coverageGaps };
  };
  ...
  const createFromTemplate = async (templateId: string, customizations?: Partial<Control>): Promise<Control> => {
