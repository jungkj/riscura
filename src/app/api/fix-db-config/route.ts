...
export async function GET() {
  const config = {
    current: {
      ...
      actualUrl: (process.env.DATABASE_URL || process.env.database_url || '').substring(0, 60) + '...',
      directUrl: (process.env.DIRECT_URL || '').substring(0, 60) + '...',
    },
    analysis: {
      isDATABASE_URLDirect: process.env.DATABASE_URL?.includes('db.') && process.env.DATABASE_URL?.includes('.supabase.co'),
  ...
  if (process.env.DIRECT_URL && process.env.DATABASE_URL === process.env.DIRECT_URL) {
    ...
    config.analysis.issue = 'DATABASE_URL and DIRECT_URL are the same (both using direct connection)';
  ...
  const yourPooledUrl = 'postgresql://postgres.zggstcxinvxsfksssdyr:Gynaha2pf!123@aws-0-us-east-1.pooler.supabase.com:6543/postgres';
    ...
    solution: {
      ...
      steps: [
        ...
        },
        {
          ...
          value: 'postgresql://postgres:[password]@db.zggstcxinvxsfksssdyr.supabase.co:5432/postgres',
        },
        {
          step: 4,
