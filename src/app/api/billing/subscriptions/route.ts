...
export const DELETE = withApiMiddleware(async (request: NextRequest) => {
  ...
  // Cancel subscription
  const canceledSubscription = await billingManager.cancelSubscription(subscriptionId, immediately);

  return createAPIResponse({ 
    ...canceledSubscription,
    message: immediately ? 'Subscription canceled immediately' : 'Subscription scheduled for cancellation'
  });
}, {
  requireAuth: true
});