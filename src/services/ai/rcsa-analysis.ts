...
function mapRiskCategory(category?: string): RiskCategory {
  ...
  if (lower.includes('cyber') || lower.includes('it') || lower.includes('technology')) return RiskCategory.IT_CYBER;
...
export async function analyzeRCSAData(rows: RCSARowData[]): Promise<RCSAGapAnalysis> {
  try {
    ...
    mappedControls.forEach((control) => {
      ...
      if (missingFields.length > 0) {
        controlGaps.push({
          ...
          severity: missingFields.includes('Control Owner') || missingFields.includes('Control Evidence') ? 'high' : 'medium',
    ...
    return {
      overallAssessment: aiResponse.overallAssessment || `RCSA data shows ${completenessScore}% completeness with ${riskGaps.length} risk gaps and ${controlGaps.length} control gaps identified.`,
      completenessScore,
      riskGaps,
      controlGaps,
      recommendations,
