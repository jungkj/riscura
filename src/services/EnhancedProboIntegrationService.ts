...
export class EnhancedProboIntegrationService {
  ...
  private generateTags(_mitigation: any): string[] {
    ...
    // Add framework tags
    const frameworks = mitigation.standards
      .split(';')
  ...
  private isRelevantForRisk(_mitigation: any, request: ControlGenerationRequest): boolean {
    ...
    const categoryMatch = mitigation.category
      .toLowerCase()
      .includes(request.riskCategory.toLowerCase());
  ...
  private createRiskControlMappings(
    riskId: string,
  ...
  private determineMappingType(
    ...
    if (
      control.name.toLowerCase().includes('monitor') ||
      control.name.toLowerCase().includes('detect')
    ) {
      return 'Detective';
    }
    if (
      control.name.toLowerCase().includes('response') ||
      control.name.toLowerCase().includes('recover')
    ) {
      return 'Corrective';
    }
    return 'Preventive';
