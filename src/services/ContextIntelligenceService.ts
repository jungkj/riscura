...
export class ContextIntelligenceService {
  ...
  private generateMinimalContext(
    ...
    if (context.current.selectedEntities.risks.length > 0) {
      parts.push(`Current risks: ${context.current.selectedEntities.risks.map(r => r.title).join(', ')}`);
    }
    
    if (context.current.selectedEntities.controls.length > 0) {
      parts.push(`Current controls: ${context.current.selectedEntities.controls.map(c => c.title).join(', ')}`);
    }
    
    parts.push(`User role: ${context.user.role}`);
    parts.push(`Company: ${context.organizational.companyProfile.name} (${context.organizational.companyProfile.industry})`);
  ...
  private generateModerateContext(
    ...
    if (context.current.selectedEntities.controls.length > 0) {
      ...
      context.current.selectedEntities.controls.forEach(control => {
        parts.push(`- ${control.title} (${control.type}, Effectiveness: ${control.effectiveness}%)`);
      });
    }
    ...
    parts.push(`- Key Regulations: ${context.organizational.complianceRequirements.mandatory.join(', ')}`);
  ...
  private generateComprehensiveContext(
    ...
    if (context.user.recentActivity.length > 0) {
      ...
      context.user.recentActivity.slice(0, 5).forEach(activity => {
        additionalParts.push(`- ${activity.action} on ${activity.entityType} (${activity.timestamp.toLocaleDateString()})`);
      });
    }
  ...
  private async generateDataInsightSuggestions(context: IntelligentContext): Promise<SmartContextSuggestion[]> {
    const suggestions: SmartContextSuggestion[] = [];
  ...
  private async generateMissingContextSuggestions(context: IntelligentContext): Promise<SmartContextSuggestion[]> {
    const suggestions: SmartContextSuggestion[] = [];
    
    if (context.current.selectedEntities.risks.length === 0 && context.current.page.includes('risk')) {
      suggestions.push({
  ...
  private async generateRelatedEntitySuggestions(context: IntelligentContext): Promise<SmartContextSuggestion[]> {
    const suggestions: SmartContextSuggestion[] = [];
    
    if (context.current.selectedEntities.risks.length > 0 && context.current.selectedEntities.controls.length === 0) {
      suggestions.push({
  ...
  private async generateActionItemSuggestions(context: IntelligentContext): Promise<SmartContextSuggestion[]> {
    const suggestions: SmartContextSuggestion[] = [];
  ...
  private async generateEfficiencySuggestions(context: IntelligentContext): Promise<SmartContextSuggestion[]> {
    const suggestions: SmartContextSuggestion[] = [];
